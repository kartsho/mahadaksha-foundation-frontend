...
function ServiceCard({ service }) {
  return (
    ...
    >
      <h4 className="mb-3 text-lg font-bold text-white">{service.name}</h4>
...
export default function Services() {
  ...
  const totalServicesOffered = useMemo(
    () => servicesData.reduce((sum, c) => sum + c.count, 0),
    []
  );

  const activeCategory = servicesData.find((c) => c.slug === activeSlug);
      ...
      <div className="mx-auto mt-12 max-w-6xl">
        {activeCategory && (
          <CategorySection
            ...
            index={servicesData.findIndex((c) => c.slug === activeSlug)}
          />
        )}
      </div>
    </div>
